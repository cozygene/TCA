...
test_that("Comparison with the results given by the matlab version", {
  ...
  # run tca
  res <- tca(t(X), W, C1 = C1, C2 = C2, refit_W = TRUE, refit_t = ncol(X), parallel = FALSE, log_file = NULL)
  ...
  res2 <- tca(t(X), W_noisy, C1 = C1, C2 = C2, refit_W = TRUE, refit_t = ncol(X), parallel = FALSE)
